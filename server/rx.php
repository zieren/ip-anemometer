...
function handleRequest() {
  ...
  try {
    ...
    $logger->debug('client md5: '.$meta['md5'].' - server has: '.$config['md5']);
    // TODO: Server should lazily create md5.
    ...
    if (isset($meta['md5']) && $meta['md5'] != NOT_AVAILABLE
        && isset($config['md5']) && $meta['md5'] != $config['md5']) {
      $logger->notice('updating client '.$meta['md5'].' to '.$config['md5']);
